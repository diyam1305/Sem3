...
Create database Bank_System_143
...
(
	  ...
	  References Bank_Master (Bank_ID) on 
...
(
	   ...
	   Emp_Salary float,
	   Constraint FK_Branch_IFSC Foreign key (Branch_IFSC) 
	   References Branch_Master (Branch_IFSC) on 
	   Delete Cascade on Update Cascade
)
...
(
	   ...
	   References Customer_Master (Cust_ID) on 
	   Delete Cascade on Update Cascade,
	   Constraint FK_Branch2_IFSC Foreign key (Branch_IFSC) 
	   References Branch_Master (Branch_IFSC) on 
	   Delete Cascade on Update Cascade,
...
(
	   ...
	   Tran_Amount FLOAT,
	   Constraint FK_Tran_Acc_No Foreign key (Tran_Acc_No) 
	   References Account_Master (Acc_No) on 
	   Delete Cascade on Update Cascade,
	   Constraint Check_Tran_Type Check (Tran_Type in ('CH','CQ','OL','RG')),
	   Constraint CheckTran_Amount_Debit_Credit 
	   Check (Tran_Amount_Debit_Credit in ('C','D'))
)
...
values(201,'Green Chowk Morbi','SBI101MB201',101),
	  ...
	  (206,'Lal Darwaja Ahemdabad, Rajkot','BOB103AM206',103),
...
values(401,'C N Kanani','1985-08-20','Milap Nagar,University Road',2134567890,'cnk@gmail.com','Gandhinagar'),
	  ...
	  (405,'K B Bhalodia','1996-03-25','Nanavati Chowk,University Road',1234568790,'kbb@gmail.com','Rajkot'),
